...
class InnerAlertController: UIAlertController {
    ...
    private var imageTopMargin: Alertify.ImageTopMargin = .none
    ...
    public func setImage(_ image: UIImage?, imageTopMargin: Alertify.ImageTopMargin = .none) {
    ...
    var actionHandler: (UIAlertAction) -> (UIAlertAction, Int) {
        return { [weak self] action in
            (action, self?.actions.index(of: action) ?? -1)
        }
    }
    ...
    var actionWithTextFieldsHandler: (UIAlertAction) -> (UIAlertAction, Int, [UITextField]?) {
        return { [weak self] action in
            (action, self?.actions.index(of: action) ?? -1, self?.textFields)            
        }
    }
